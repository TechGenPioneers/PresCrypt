...
export default function AppointmentsPage() {
  ...
  const [noAppointments, setNoAppointments] = useState(false); // Track open menu
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      ...
      try {
        ...
        console.log("Formatted Date:", formattedDate); // Log formatted date for verification

        const response = await axiosInstance.get(
          `/api/Appointments/date/${formattedDate}?doctorId=1` //replace with dynamically changing id
        );
        setAppointments(response.data);

        // Check if the appointments array is empty
        if (response.data.length === 0) {
          console.log("No appointments found for the selected date.");
          // Optionally, set a state to display a message to the user
