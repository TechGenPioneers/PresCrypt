"use client";
import Message from "./message";

export default function MessageList({ messages, messagesEndRef }) {
return (
    <div className="p-4 overflow-y-auto h-[calc(100%-130px)] bg-white-50">
        <div className="flex justify-center mb-4">
            <div className="w-76 h-55 flex items-center justify-center">
                <img 
                    src="/195.jpg" 
                    alt="Chatbot" 
                    className="h-40 w-60 animate-bounce"
                    onError={(e) => {
                        e.target.onerror = null;
                        e.target.src = "data:image/svg+xml;base64,";
                    }}
                />
            </div>
        </div>

        {messages.map((message, index) => (
            <div
                key={index}
                className="animate-fade-in"
                style={{ animationDelay: `${index * 0.1}s` }}
            >
                <Message message={message} />
            </div>
        ))}
        
        <div ref={messagesEndRef} />
    </div>
);
}